...
namespace {
    ...
    std::vector<Vector3> GetVertices(const OBB& obb) {
        ...
        for (size_t i = 0; i < vertices.size(); ++i) {
            vertices[i] = Transform(vertices[i] , obbWorldMatrix);
        }
...
void OBBCollider::DrawCollision(const ViewProjection& viewProjection,const Vector4& color) {
    ...
    corners[0] = obb_.center - obb_.orientations[0] * obb_.size.x - obb_.orientations[1] * obb_.size.y - obb_.orientations[2] * obb_.size.z;
    corners[1] = obb_.center + obb_.orientations[0] * obb_.size.x - obb_.orientations[1] * obb_.size.y - obb_.orientations[2] * obb_.size.z;
    corners[2] = obb_.center + obb_.orientations[0] * obb_.size.x + obb_.orientations[1] * obb_.size.y - obb_.orientations[2] * obb_.size.z;
    corners[3] = obb_.center - obb_.orientations[0] * obb_.size.x + obb_.orientations[1] * obb_.size.y - obb_.orientations[2] * obb_.size.z;
    corners[4] = obb_.center - obb_.orientations[0] * obb_.size.x - obb_.orientations[1] * obb_.size.y + obb_.orientations[2] * obb_.size.z;
    corners[5] = obb_.center + obb_.orientations[0] * obb_.size.x - obb_.orientations[1] * obb_.size.y + obb_.orientations[2] * obb_.size.z;
    corners[6] = obb_.center + obb_.orientations[0] * obb_.size.x + obb_.orientations[1] * obb_.size.y + obb_.orientations[2] * obb_.size.z;
    corners[7] = obb_.center - obb_.orientations[0] * obb_.size.x + obb_.orientations[1] * obb_.size.y + obb_.orientations[2] * obb_.size.z;

    // Draw the edges of the OBB
    for (const auto& edge : edges) {
        drawLine->SetLine(corners[edge.first], corners[edge.second], color);
    }
