...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    // スペキュラー
    float blinnPhongReflection = BlinnPhongReflection(input.normal, gViewProjection.cameraPos, worldPos, gDirectionLight.direction, gDirectionLight.sharpness);
    float3 specular = float3(gDirectionLight.color.rgb * blinnPhongReflection * float3(1.0f, 1.0f, 1.0f));
    // アンビエント
