...
void AttractionField(FieldForGPU field,inout Particle particle){
    ...
    if(field.fieldArea.type==0){
        direction = field.fieldArea.aabb.position - particle.translate;
    }else if(field.fieldArea.type==1){
        direction = field.fieldArea.sphere.position - particle.translate;
    }else if(field.fieldArea.type==2){
        float32_t3 position=closestPointOnSegment(particle.translate,field.fieldArea.capsule.segment.origin,field.fieldArea.capsule.segment.diff);
        direction =  position - particle.translate;
    }
    ...
    if(origalField[fieldIndex].isAlive&&particle[particleIndex].isAlive){
        if((particle[particleIndex].collisionInfo.attribute & origalField[fieldIndex].collisionInfo.mask)!=0){
            ...
            if(origalField[fieldIndex].fieldArea.type==0){
                if(sdAABB(particle[particleIndex].translate,origalField[fieldIndex].fieldArea.aabb.position,origalField[fieldIndex].fieldArea.aabb.range.min,origalField[fieldIndex].fieldArea.aabb.range.max) <= 0){
                    UpdateField(origalField[fieldIndex],particle[particleIndex]);
                }
            }
            // Sphere
            else if(origalField[fieldIndex].fieldArea.type==1){
                if(sdSphere(particle[particleIndex].translate,origalField[fieldIndex].fieldArea.sphere.position,origalField[fieldIndex].fieldArea.sphere.radius) <= 0){
                    UpdateField(origalField[fieldIndex],particle[particleIndex]);
                }
            }
            // Capsule
            else if(origalField[fieldIndex].fieldArea.type==2){
                if(sdCapsule(particle[particleIndex].translate,origalField[fieldIndex].fieldArea.capsule.segment.origin,origalField[fieldIndex].fieldArea.capsule.segment.diff,origalField[fieldIndex].fieldArea.capsule.radius) <= 0){
                    UpdateField(origalField[fieldIndex],particle[particleIndex]);
                }
            }
        }
